...
describe(testUtils.getTestName(__filename), () => {
  it('return custom error', function it() {
    ...
    const result = new ForbiddenError(message);

    expect(result).to.have.property('status');
    expect(result.status).to.be.eql(HTTP_CODES.FORBIDDEN);

    const expectedProps = {
      message,
      name: HTTP_ERRORS[HTTP_CODES.FORBIDDEN]
    };
  ...
  it('return error with default message', function it() {
    const result = new ForbiddenError();
    ...
    const expectedProps = {
      name   : HTTP_ERRORS[HTTP_CODES.FORBIDDEN],
      message: HTTP_ERRORS[HTTP_CODES.FORBIDDEN]
    };
